...
class TikTokLiveConnection extends node_events_1.EventEmitter {
    ...
    async fetchRoomId(uniqueId) {
        ...
        try {
            ...
            if (!roomId)
                throw new Error('Failed to extract roomId from API.');
            return roomId;
        }
        catch (err) {
        ...
        try {
            ...
            if (!response.ok)
                throw new Error(`Failed to retrieve roomId from Euler due to an error: ${response.message}`);
            if (!response.room_id)
                throw new Error('Failed to extract roomId from Euler.');
